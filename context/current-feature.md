...
- **2026-06-25** — Completed Auth Phase 1: NextAuth v5 + GitHub OAuth — split config pattern (`auth.config.ts` + `auth.ts`), Prisma adapter with JWT strategy, catch-all API route, `src/proxy.ts` protecting `/dashboard/*` with redirect to sign-in, Session type extended with `user.id`
