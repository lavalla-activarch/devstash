...
export default async function DashboardPage() {
  const [collections, itemStats, pinnedItems, recentItems] = await Promise.all([
    ...
    getItemStats(DEMO_USER_EMAIL),
    getPinnedItems(DEMO_USER_EMAIL),
    getRecentItems(DEMO_USER_EMAIL),
  ]);
  ...
  return (
    <div className="p-6 space-y-8 max-w-5xl">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard icon={Package} label="Total Items" value={itemStats.totalItems} />
        <StatCard icon={Layers} label="Collections" value={collections.length} />
        <StatCard icon={Heart} label="Favorite Items" value={itemStats.favoriteItems} />
